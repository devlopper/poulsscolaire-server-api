...
class AccountingAccountSchoolClientTest {

  AccountingAccountSchoolService service;
  AccountingAccountSchoolClient client;

  @BeforeEach
  void listenBeforeEach() {
    service = Mockito.mock(AccountingAccountSchoolService.class);
    client = new AccountingAccountSchoolClient().service(service);
  }
  ...
  void create() {
    ...
    assertInstanceOf(CreateResponseDto.class,
        client.create(new AccountingAccountSchoolCreateRequestDto()));
  }
  ...
  void getMany() {
    ...
    Mockito.when(response.readEntity(AccountingAccountSchoolGetManyResponseDto.class))
        .thenReturn(new AccountingAccountSchoolGetManyResponseDto());
    Mockito.when(service.getMany(any())).thenReturn(response);
    assertInstanceOf(AccountingAccountSchoolGetManyResponseDto.class,
  ...
  void getOne() {
    Response response = Mockito.mock(Response.class);
    Mockito.when(response.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    Mockito.when(response.readEntity(AccountingAccountSchoolDto.class))
        .thenReturn(new AccountingAccountSchoolDto());
    Mockito.when(service.getOne(any())).thenReturn(response);
    assertInstanceOf(AccountingAccountSchoolDto.class, client.getOne(null, null, null, null));
  }
  ...
  void getByIdentifier() {
    ...
    assertInstanceOf(AccountingAccountSchoolDto.class,
  ...
  void update() {
    ...
    assertInstanceOf(IdentifiableResponseDto.class,
        client.update(new AccountingAccountSchoolUpdateRequestDto()));
  }
