...
class AccountingAccountClientTest {
  ...
  void listenBeforeEach() {
    service = Mockito.mock(AccountingAccountService.class);
    client = new AccountingAccountClient().service(service);
  }
  ...
  void create() {
    ...
    assertInstanceOf(CreateResponseDto.class,
        client.create(new AccountingAccountCreateRequestDto()));
  }
  ...
  void getMany() {
    ...
    Mockito.when(response.readEntity(AccountingAccountGetManyResponseDto.class))
        .thenReturn(new AccountingAccountGetManyResponseDto());
    Mockito.when(service.getMany(any())).thenReturn(response);
    assertInstanceOf(AccountingAccountGetManyResponseDto.class,
  ...
  void getOne() {
    Response response = Mockito.mock(Response.class);
    Mockito.when(response.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    Mockito.when(response.readEntity(AccountingAccountDto.class))
        .thenReturn(new AccountingAccountDto());
    Mockito.when(service.getOne(any())).thenReturn(response);
    assertInstanceOf(AccountingAccountDto.class, client.getOne(null, null, null, null));
  }
  ...
  void getByIdentifier() {
    ...
    assertInstanceOf(AccountingAccountDto.class, client.getByIdentifier(null, null, null, null));
  }
  ...
  void update() {
    ...
    assertInstanceOf(IdentifiableResponseDto.class,
        client.update(new AccountingAccountUpdateRequestDto()));
  }
