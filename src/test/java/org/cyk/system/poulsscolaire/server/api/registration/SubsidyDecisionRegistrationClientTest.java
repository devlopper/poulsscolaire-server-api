...
class SubsidyDecisionRegistrationClientTest {

  SubsidyDecisionRegistrationService service;
  SubsidyDecisionRegistrationClient client;
  SubsidyDecisionRegistrationRequestMapper mapper =
      Mappers.getMapper(SubsidyDecisionRegistrationRequestMapper.class);

  @BeforeEach
  void listenBeforeEach() {
    service = Mockito.mock(SubsidyDecisionRegistrationService.class);
    client = new SubsidyDecisionRegistrationClient().service(service);
  }
  ...
  void create() {
    ...
    assertInstanceOf(CreateResponseDto.class,
        client.create(new SubsidyDecisionRegistrationCreateRequestDto()));
  }
  ...
  void getMany() {
    ...
    Mockito.when(response.readEntity(SubsidyDecisionRegistrationGetManyResponseDto.class))
        .thenReturn(new SubsidyDecisionRegistrationGetManyResponseDto());
    Mockito.when(service.getMany(any())).thenReturn(response);
    assertInstanceOf(SubsidyDecisionRegistrationGetManyResponseDto.class,
  ...
  void getOne() {
    Response response = Mockito.mock(Response.class);
    Mockito.when(response.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    Mockito.when(response.readEntity(SubsidyDecisionRegistrationDto.class))
        .thenReturn(new SubsidyDecisionRegistrationDto());
    Mockito.when(service.getOne(any())).thenReturn(response);
    assertInstanceOf(SubsidyDecisionRegistrationDto.class, client.getOne(null, null, null, null));
  }
  ...
  void getByIdentifier() {
    ...
    assertInstanceOf(SubsidyDecisionRegistrationDto.class,
  ...
  void update() {
    ...
    assertInstanceOf(IdentifiableResponseDto.class,
        client.update(new SubsidyDecisionRegistrationUpdateRequestDto()));
  }
  ...
  void filter_doInitialize() {
    SubsidyDecisionRegistrationFilter filter = new SubsidyDecisionRegistrationFilter(null);
  ...
  @Test
  void filter_toDto() {
    SubsidyDecisionRegistrationFilter filter = new SubsidyDecisionRegistrationFilter();
    FilterDto dto = filter.toDto();
    assertNotNull(dto);
  }
  ...
  void mapper_mapCreation_whenNotNull() {
    assertNotNull(mapper.mapCreate(new SubsidyDecisionRegistrationDto()));
  }
  ...
  void mapper_mapUpdate_whenNotNull() {
    assertNotNull(mapper.mapUpdate(new SubsidyDecisionRegistrationDto()));
  }
}
