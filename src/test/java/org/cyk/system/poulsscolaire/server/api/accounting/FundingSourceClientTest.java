...
class FundingSourceClientTest {
  ...
  void listenBeforeEach() {
    service = Mockito.mock(FundingSourceService.class);
    client = new FundingSourceClient().service(service);
  }
  ...
  void create() {
    ...
    assertInstanceOf(CreateResponseDto.class,
        client.create(new FundingSourceCreateRequestDto()));
  }
  ...
  void getMany() {
    ...
    Mockito.when(response.readEntity(FundingSourceGetManyResponseDto.class))
        .thenReturn(new FundingSourceGetManyResponseDto());
    Mockito.when(service.getMany(any())).thenReturn(response);
    assertInstanceOf(FundingSourceGetManyResponseDto.class,
  ...
  void getOne() {
    Response response = Mockito.mock(Response.class);
    Mockito.when(response.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    Mockito.when(response.readEntity(FundingSourceDto.class))
        .thenReturn(new FundingSourceDto());
    Mockito.when(service.getOne(any())).thenReturn(response);
    assertInstanceOf(FundingSourceDto.class, client.getOne(null, null, null, null));
  }
  ...
  void getByIdentifier() {
    ...
    assertInstanceOf(FundingSourceDto.class, client.getByIdentifier(null, null, null, null));
  }
  ...
  void update() {
    ...
    assertInstanceOf(IdentifiableResponseDto.class,
        client.update(new FundingSourceUpdateRequestDto()));
  }
