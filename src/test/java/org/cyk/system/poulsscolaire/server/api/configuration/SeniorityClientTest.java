...
class SeniorityClientTest {

  SeniorityService service;
  SeniorityClient client;

  @BeforeEach
  void listenBeforeEach() {
    service = Mockito.mock(SeniorityService.class);
    client = new SeniorityClient().service(service);
  }
  ...
  void getMany() {
    ...
    Mockito.when(response.readEntity(SeniorityGetManyResponseDto.class))
        .thenReturn(new SeniorityGetManyResponseDto());
    Mockito.when(service.getMany(any())).thenReturn(response);
    assertInstanceOf(SeniorityGetManyResponseDto.class,
  ...
  void getOne() {
    Response response = Mockito.mock(Response.class);
    Mockito.when(response.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    Mockito.when(response.readEntity(SeniorityDto.class)).thenReturn(new SeniorityDto());
    Mockito.when(service.getOne(any())).thenReturn(response);
    assertInstanceOf(SeniorityDto.class, client.getOne(null, null, null, null));
  }
  ...
  void getByIdentifier() {
    ...
    assertInstanceOf(SeniorityDto.class, client.getByIdentifier(null, null, null, null));
  }
